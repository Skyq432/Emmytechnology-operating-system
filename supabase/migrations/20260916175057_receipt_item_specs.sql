-- Receipts only ever printed item_name/quantity/amount, even though non-stock
-- order lines (laptop/phone/etc.) already capture real specs (RAM, storage,
-- serial number, ...) in ops_order_items.specs. Include item_type and specs in
-- the item snapshot so the receipt PDF can print them under the item name.
...
declare
  ...
  v_number text;
begin
  if not public.staff_has_any_capability(array['sales.direct.manage','sales.order.manage','operations.order.manage']) then raise exception 'Not authorized'; end if;
  ...
  if v_order.branch_location_id is not null then select * into v_location from public.ops_locations where id=v_order.branch_location_id; end if;
  ...
  if v_paid<coalesce(v_order.total_amount,0) then raise exception 'Final receipt requires full payment of the amount payable'; end if;
  select coalesce(jsonb_agg(jsonb_build_object('item_name',item_name,'item_type',item_type,'specs',specs,'quantity',quantity,'unit_price',unit_price,'list_price',list_price,'line_discount_amount',line_discount_amount,'line_total',line_total) order by created_at,id),'[]'::jsonb) into v_items from public.ops_order_items where order_id=p_order_id;
  select coalesce(jsonb_agg(jsonb_build_object('amount',amount,'payment_method',payment_method,'reference',reference,'paid_at',paid_at) order by paid_at,created_at,id),'[]'::jsonb) into v_payments from public.ops_order_payments where order_id=p_order_id and not is_void;
  ...
  values(v_number,'final_sales_receipt',v_order.identity_id,v_order.id,jsonb_build_object('source_type','order','source_code',v_order.order_code,'customer_name',v_order.customer_name,'customer_phone',v_order.customer_phone,'customer_email',v_order.customer_email,'items',v_items,'subtotal',v_order.subtotal,'discount_amount',v_order.discount_amount,'delivery_charge',v_order.delivery_charge,'gross_amount',v_gross,'cash_off_amount',coalesce(v_order.cash_off_amount,0),'amount_payable',v_order.total_amount,'transaction_total',v_order.total_amount,'payments',v_payments,'cash_payment_total',v_paid,'total_paid',v_paid,'effective_paid',v_effective_paid,'balance_due',0,'branch_location_id',v_order.branch_location_id,'branch_code',v_location.code,'branch_name',v_location.name,'handled_by_user_id',v_order.handled_by_user_id,'handled_by_name',v_order.handled_by_name,'sales_staff_user_id',v_order.sales_staff_user_id,'sales_staff_name',coalesce(v_order.sales_staff_name,v_order.handled_by_name)),auth.uid()) returning * into v_doc;
  perform public.sales_prepare_document_deliveries(v_doc.id,v_order.customer_email);
  insert into public.sales_events(identity_id,order_id,event_type,title,metadata,actor_id) values(v_order.identity_id,v_order.id,'receipt.issued','Final sales receipt queued',jsonb_build_object('document_id',v_doc.id,'document_number',v_doc.document_number),auth.uid());
  return v_doc;
...
CREATE OR REPLACE FUNCTION public.sales_ensure_payment_receipt_metadata(p_source_type text, p_source_payment_id uuid)
 RETURNS sales_documents
 LANGUAGE plpgsql
 SECURITY DEFINER
...
declare
  ...
  v_number text;
begin
  if not public.staff_has_capability('sales.payment.record') then raise exception 'Not authorized'; end if;
  ...
  if p_source_type='order' then
    ...
    select coalesce(jsonb_agg(jsonb_build_object(
      'item_name',item_name,'item_type',item_type,'specs',specs,'quantity',quantity,'unit_price',unit_price,'list_price',list_price,
    ...
    insert into public.sales_documents(
      ...
      jsonb_build_object(
        ...
        'device_type',v_repair.device_type,'brand',v_repair.brand,'model',v_repair.model,'serial_or_imei',v_repair.serial_or_imei,
