...
begin
  ...
  values(p_repair_id,'technical_details_updated','Repair work details updated','staff',auth.uid(),'{}'::jsonb);
end;
$$;
...
create policy "inventory staff read suppliers" on public.ops_suppliers for select to authenticated using (public.staff_has_capability('operations.inventory.read'));
