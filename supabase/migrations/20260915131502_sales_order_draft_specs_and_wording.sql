-- Non-stock order lines can now carry category-driven spec details (captured on the
-- order line itself, same shape as ops_order_items.specs already used by the
-- Operations "Sales details" panel) instead of nothing. The INSERT previously never
-- included specs at all. Also reword the pricing-incomplete error now that the
-- frontend no longer calls these "Service" items.
