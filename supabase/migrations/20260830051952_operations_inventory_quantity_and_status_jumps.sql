...
as $$
declare
  v_item public.ops_inventory_items%rowtype;
begin
  ...
  if coalesce(p_opening_quantity,0) > 0 then
    perform public.ops_create_stock_movement(v_item.id,p_opening_location_id,'opening_balance',p_opening_quantity,'inventory_item',v_item.id,'Opening stock recorded when item was created');
  end if;
...
begin
  ...
  v_id := public.ops_create_stock_movement(
    p_inventory_item_id,p_location_id,'stock_received',p_quantity,
...
create or replace function public.ops_change_order_status(p_order_id uuid,p_new_status text,p_note text default null)
returns void
language plpgsql
security definer
set search_path=public
