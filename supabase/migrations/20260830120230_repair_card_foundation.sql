...
begin
  ...
  values(v_repair_id, v_assignment_id, 'repair_received', 'Repair received', true, auth.uid(), jsonb_build_object('card_code', v_card.card_code));

  return jsonb_build_object('repair_id',v_repair_id,'repair_code',v_repair_code,'assignment_id',v_assignment_id,'card_id',v_card.id,'card_code',v_card.card_code,'access_pin',p_access_pin);
end;
$$;
...
begin
  ...
  v_required := case when p_payment_requirement='none' then 0 when p_payment_requirement='full' then p_quote_amount else greatest(coalesce(p_required_before_start,0),0) end;
  if v_required > p_quote_amount then raise exception 'Required start payment cannot exceed quote amount'; end if;
  select coalesce(max(version),0)+1 into v_version from public.ops_repair_quotes where repair_id=p_repair_id;
  update public.ops_repair_quotes set status='superseded' where repair_id=p_repair_id and status in ('draft','published');
  insert into public.ops_repair_quotes(repair_id,version,diagnosis_public,work_description,quote_amount,estimated_completion,payment_requirement,required_before_start,status,published_at,created_by)
  values(p_repair_id,v_version,nullif(btrim(p_diagnosis_public),''),nullif(btrim(p_work_description),''),p_quote_amount,nullif(btrim(p_estimated_completion),''),p_payment_requirement,v_required,'published',now(),auth.uid()) returning id into v_quote_id;
  v_balance := greatest(p_quote_amount-coalesce(v_repair.amount_paid,0),0);
  v_payment_status := case when p_quote_amount>0 and coalesce(v_repair.amount_paid,0)>=p_quote_amount then 'paid' when coalesce(v_repair.amount_paid,0)>0 then 'partial' else 'unpaid' end;
  update public.ops_repairs set current_quote_id=v_quote_id,amount_charged=p_quote_amount,balance_due=v_balance,payment_status=v_payment_status,status='awaiting_customer_approval' where id=p_repair_id;
  insert into public.ops_repair_events(repair_id,assignment_id,event_type,title,from_status,to_status,customer_visible,actor_id,metadata)
  values(p_repair_id,v_repair.current_card_assignment_id,'quote_published','Repair quote ready for approval',v_repair.status,'awaiting_customer_approval',true,auth.uid(),jsonb_build_object('quote_id',v_quote_id,'version',v_version,'quote_amount',p_quote_amount,'payment_requirement',p_payment_requirement,'required_before_start',v_required));
  return jsonb_build_object('quote_id',v_quote_id,'version',v_version,'quote_amount',p_quote_amount,'required_before_start',v_required,'balance_due',v_balance,'payment_status',v_payment_status);
end;
$$;
...
begin
  ...
  v_status := case when v_total>0 and v_paid>=v_total then 'paid' when v_paid>0 then 'partial' else 'unpaid' end;
  v_gate_satisfied := v_quote.id is not null and v_quote.status='approved' and v_paid>=v_quote.required_before_start;
  update public.ops_repairs set amount_paid=v_paid,balance_due=v_balance,payment_status=v_status where id=p_repair_id;
  insert into public.ops_repair_events(repair_id,assignment_id,event_type,title,customer_visible,actor_id,metadata)
  values(p_repair_id,v_repair.current_card_assignment_id,'payment_recorded','Repair payment recorded',true,auth.uid(),jsonb_build_object('payment_id',v_payment_id,'amount',p_amount,'amount_paid',v_paid,'balance_due',v_balance,'payment_status',v_status,'start_gate_satisfied',v_gate_satisfied));
  return jsonb_build_object('payment_id',v_payment_id,'amount_paid',v_paid,'balance_due',v_balance,'payment_status',v_status,'start_gate_satisfied',v_gate_satisfied);
end;
$$;
...
begin
  ...
  if v_repair.current_quote_id is not null then select * into v_quote from public.ops_repair_quotes where id=v_repair.current_quote_id; end if;
  v_gate_ok := v_quote.id is not null and v_quote.status='approved' and coalesce(v_repair.amount_paid,0)>=v_quote.required_before_start;
  ...
  insert into public.ops_repair_events(repair_id,assignment_id,event_type,title,note,from_status,to_status,customer_visible,actor_id)
  values(p_repair_id,v_repair.current_card_assignment_id,'status_changed','Repair status updated',nullif(btrim(p_note),''),v_repair.status,p_new_status,p_new_status in ('diagnosing','awaiting_customer_approval','awaiting_payment','awaiting_parts','in_progress','quality_check','ready_collection','rework'),auth.uid());
  return jsonb_build_object('status',p_new_status,'start_gate_satisfied',v_gate_ok);
end;
$$;

create or replace function public.ops_regenerate_repair_pin(p_repair_id uuid,p_new_pin text)
returns jsonb language plpgsql security definer set search_path = public as $$
declare
  v_repair public.ops_repairs%rowtype;
...
begin
  ...
  update public.ops_repair_card_assignments set access_pin=p_new_pin,pin_version=pin_version+1,handover_started_at=null,handover_expires_at=null where id=v_assignment.id returning pin_version into v_pin_version;
  ...
  insert into public.ops_repair_events(repair_id,assignment_id,event_type,title,customer_visible,actor_id,metadata)
  values(p_repair_id,v_assignment.id,'pin_regenerated','Repair access PIN regenerated',false,auth.uid(),jsonb_build_object('pin_version',v_pin_version,'card_code',v_card_code));
  return jsonb_build_object('assignment_id',v_assignment.id,'card_code',v_card_code,'access_pin',p_new_pin,'pin_version',v_pin_version);
end;
$$;
...
begin
  ...
  select * into v_assignment from public.ops_repair_card_assignments where id=v_repair.current_card_assignment_id and status='active' for update;
  if not found then raise exception 'Active Repair Card assignment not found'; end if;
  update public.ops_repair_card_assignments set handover_started_at=now(),handover_expires_at=v_expires where id=v_assignment.id;
  insert into public.ops_repair_events(repair_id,assignment_id,event_type,title,customer_visible,actor_id,metadata)
  values(p_repair_id,v_assignment.id,'handover_started','Customer handover started',true,auth.uid(),jsonb_build_object('expires_at',v_expires));
  return jsonb_build_object('assignment_id',v_assignment.id,'handover_expires_at',v_expires);
end;
$$;

create or replace function public.ops_complete_repair_collection(p_repair_id uuid,p_card_returned boolean,p_missing_card_reason text default null)
returns jsonb language plpgsql security definer set search_path = public as $$
declare
  v_repair public.ops_repairs%rowtype;
  v_assignment public.ops_repair_card_assignments%rowtype;
...
begin
  ...
  select * into v_assignment from public.ops_repair_card_assignments where id=v_repair.current_card_assignment_id and status='active' for update;
  if not found then raise exception 'Active Repair Card assignment not found'; end if;
  select exists(select 1 from public.ops_repair_consents where repair_id=p_repair_id and assignment_id=v_assignment.id and consent_type='completion_acceptance') into v_has_acceptance;
  ...
  values(p_repair_id,v_assignment.id,'repair_collected','Repair collected',case when p_card_returned then null else btrim(p_missing_card_reason) end,'ready_collection','collected',true,auth.uid(),jsonb_build_object('card_returned',coalesce(p_card_returned,false),'card_status',v_card_status));
