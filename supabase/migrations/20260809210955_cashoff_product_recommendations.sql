...
alter table public.cashoff_recommendation_settings enable row level security;
revoke all on table public.cashoff_recommendation_settings from anon, authenticated;
with chosen as (
  select array_agg(id order by slot) as ids
  from (
    select p.id,
      row_number() over (
        order by coalesce(p.featured, false) desc, p.created_at desc nulls last, p.id
      ) as slot
...
insert into public.cashoff_recommendation_settings (
  id, enabled, eyebrow, headline, body_text, product_1_id, product_2_id
)
select 1, true, 'For your Cash-Off', 'Two products worth a look',
...
declare
  ...
  v_products jsonb;
begin
  select * into v_settings from public.cashoff_recommendation_settings where id = 1;
  if not found or not v_settings.enabled then
    return jsonb_build_object('enabled', false, 'products', '[]'::jsonb);
  end if;
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
...
$$;
revoke all on function public.get_cashoff_recommendations() from public;
grant execute on function public.get_cashoff_recommendations() to anon, authenticated;
