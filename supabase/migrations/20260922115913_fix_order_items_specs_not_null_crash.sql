-- ops_order_items.specs is `jsonb not null default '{}'`. Both functions below
-- explicitly insert `nullif(v_item->'specs','null'::jsonb)` for the specs column.
-- For any stock/inventory line (which never carries a specs key at all), that
-- expression evaluates to a literal SQL NULL, and an explicitly-NULL value in an
-- INSERT always overrides the column DEFAULT — so every Direct Sale and every
-- Order with a physical stock item has been failing with
-- "null value in column "specs" of relation "ops_order_items" violates
-- not-null constraint" since the specs plumbing was added. Wrapping the same
-- expression in coalesce(...,'{}'::jsonb) restores the safe fallback for
-- missing/explicit-null specs while leaving real specs objects untouched.
...
begin
  ...
  loop
    ...
    insert into public.ops_order_items(
      ...
      coalesce(nullif(v_item->'specs','null'::jsonb), '{}'::jsonb),
...
CREATE OR REPLACE FUNCTION public.sales_create_order_draft(p_identity_id uuid, p_customer_name text, p_customer_phone text, p_customer_email text, p_items jsonb, p_sales_staff_name text DEFAULT NULL::text, p_delivery_charge numeric DEFAULT 0, p_note text DEFAULT NULL::text)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
declare
  ...
  v_approval_id uuid;
begin
  if not public.staff_has_capability('sales.order.manage') then raise exception 'Not authorized'; end if;
  if p_identity_id is null or not exists(select 1 from public.identities where id=p_identity_id) then
    raise exception 'Valid customer Identity is required';
  end if;
  if p_items is null or jsonb_typeof(p_items)<>'array' or jsonb_array_length(p_items)=0 then
  ...
  loop
    ...
    if v_inventory_id is not null then
      ...
      v_price := coalesce(nullif(v_item->>'final_unit_price','')::numeric,v_list);
      v_cost := nullif(v_item->>'cost_basis','')::numeric;
      if v_list<=0 or v_price<=0 or v_cost is null or v_cost<0 then raise exception 'Non-stock item pricing is incomplete'; end if;
      v_min_margin := null;
      if nullif(trim(v_item->>'category'),'') is not null then
        select minimum_margin_percent into v_min_margin from public.sales_margin_policies
        where policy_scope='category' and lower(trim(category))=lower(trim(v_item->>'category')) and is_active=true limit 1;
  ...
  loop
    ...
    insert into public.ops_order_items(
      order_id,inventory_item_id,item_name,item_type,specs,quantity,unit_price,list_price,
