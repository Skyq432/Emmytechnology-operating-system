...
alter table public.website_events
  add column if not exists metadata jsonb
    not null
    default '{}'::jsonb;
create index if not exists website_events_identity_created_idx on public.website_events(identity_id, created_at desc);
create index if not exists website_events_visitor_created_idx on public.website_events(visitor_id, created_at desc);
create index if not exists website_events_type_created_idx on public.website_events(event_type, created_at desc);
...
begin
  ...
  where attribution.visitor_id = trim(p_visitor_id)
  ...
  if v_lead_id is null then
    select lead.id into v_lead_id
    from public.leads lead
    where lead.visitor_id = trim(p_visitor_id)
      and lead.merged_into_lead_id is null
    order by lead.updated_at desc, lead.created_at desc
    limit 1;
  end if;
  if v_event_type = 'add_to_cart' and v_lead_id is null then
    insert into public.leads (
      ambassador_id, visitor_id, identity_id, product_id, source, source_detail,
      customer_name, customer_phone, customer_email, referral_code_used, status, lead_type,
      source_page, notes, created_at, updated_at
    )
    values (
      v_session.ambassador_id, trim(p_visitor_id), v_identity_id, p_product_id, 'website_cart',
      jsonb_build_object('created_from','website_behavior_tracking'),
      case when v_identity_id is null then 'Anonymous Cart Lead' else 'Known Website Customer' end,
      'Pending - Website', null, v_session.referral_code, 'new', 'add_to_cart',
      left(p_source_page,500),
      'Lead created after the visitor showed purchase intent by adding a product to cart.',
      now(), now()
    ) returning id into v_lead_id;
  end if;
  if v_event_type = 'add_to_cart' and v_lead_id is not null then
    update public.leads
    set identity_id = coalesce(identity_id,v_identity_id),
        visitor_id = coalesce(visitor_id,trim(p_visitor_id)),
  ...
  insert into public.website_events (
    visitor_id, identity_id, lead_id, product_id, ambassador_id, event_type, quantity,
    source_page, page_url, search_query, results_count, metadata, created_at
  )
  values (
    trim(p_visitor_id), v_identity_id, v_lead_id, p_product_id, v_session.ambassador_id,
    v_event_type, v_quantity, left(p_source_page,500), left(p_page_url,1000),
    left(p_search_query,300), p_results_count, coalesce(p_metadata,'{}'::jsonb), now()
  ...
  if v_event_type in ('product_viewed','product_quick_viewed') then
    ...
    values (trim(p_visitor_id),p_product_id,v_session.ambassador_id);
  end if;
  if v_event_type = 'add_to_cart' then
    insert into public.cart_events (visitor_id, product_id, ambassador_id, quantity)
    values (trim(p_visitor_id),p_product_id,v_session.ambassador_id,v_quantity);
  end if;
...
$$;
revoke all on function public.track_website_behavior(text,text,uuid,integer,text,text,text,integer,jsonb) from public;
grant execute on function public.track_website_behavior(text,text,uuid,integer,text,text,text,integer,jsonb) to anon, authenticated;
