-- EmmyTech staff RBAC MVP: secure role-bound invitations and staff roles.
...
-- Preserve the existing Ambassador-only RPC contract for older clients.
...
-- Public/non-invite signup is always Ambassador. Internal roles require a valid role-bound invite.
...
-- The invite row is the only authority for an invited account's role.
...
begin
  ...
  if v_role = 'ambassador' then
    ...
    insert into public.ambassadors (
      ...
      true,
      '2348146503700',
      'https://ambassador.emmytechnology.com/r/' || v_custom_code,
      null,
      '{}'::jsonb,
      0,
      0,
...
-- Validation trigger must never consume the invite; the auth trigger consumes it once.
...
-- Remove Ambassador programme identity whenever an Ambassador becomes internal staff.
...
-- Prevent direct self-service role escalation through public.users updates.
...
-- Active invite codes must not be enumerable anonymously. Public validation uses get_invite_link(code).
