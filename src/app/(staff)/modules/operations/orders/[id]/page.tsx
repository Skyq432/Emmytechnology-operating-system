...
import { DraftSalesDetails } from '@/components/operations/orders/draft-sales-details';
...
import { canTransitionOrderStatus, getOrderStatusLabel, type OrderStatus } from '@/lib/operations/domain';
...
import { getOrderPayments, getSolarInstallation } from '@/lib/operations/sales-server';

const allStatuses: OrderStatus[] = ['new','confirmed','stock_check','assigned','picking','packing','ready_dispatch','dispatched','delivered','completed','on_hold','cancelled'];
...
export default async function OperationsOrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const allowedStatuses = allStatuses.filter(
    (status) => canTransitionOrderStatus(order.status, status) && !(status === 'completed' && activeReservations.length > 0)
  );
  ...
  return (
    <div className="mx-auto max-w-[1350px]">
      ...
      {order.commercial_state === 'draft' && <section className="mb-5 rounded-xl border border-blue-200 bg-blue-50/60 p-5">
        <div className="flex items-start gap-2"><div><h2 className="font-black text-[#032489]">Review before confirmation</h2><p className="mt-1 text-sm text-slate-600">Check customer, sales details, Ambassador, commission, price and fulfilment source. Internal-stock lines must have a real stock location before confirmation.</p></div><HelpTip text="Draft is preparation only. Confirm when the sale is real and internal stock has a source location." label="About confirming" /></div>
        <form action={updateDraftAttributionAction} className="mt-4 grid gap-3 rounded-lg bg-white p-4 md:grid-cols-[1fr_180px_auto] md:items-end"><input type="hidden" name="order_id" value={order.id} /><input type="hidden" name="attribution_source" value="manual_admin" /><Field label="Ambassador"><select name="ambassador_id" defaultValue={order.ambassador_id || ''} className="input"><option value="">No Ambassador</option>{ambassadors.map((item) => <option key={item.id} value={item.id}>{item.name}{item.tag ? ` · ${item.tag}` : ''}</option>)}</select></Field><Field label="Commission %"><input name="commission_rate" type="number" min="0" step="0.01" defaultValue={Number(order.commission_rate || 0)} className="input" /></Field><button className="rounded-lg border border-blue-200 bg-blue-50 px-4 py-2.5 text-sm font-black text-[#032489]">Save attribution</button></form>
        <div className="mt-4 flex flex-col justify-between gap-3 md:flex-row md:items-center"><p className="text-sm text-slate-600">Total <strong>{money(order.total_amount)}</strong>{order.ambassador_id ? ` · Estimated commission ${money(estimatedCommission)}` : ' · No Ambassador commission'}</p><form action={confirmOrderAction}><input type="hidden" name="order_id" value={order.id} /><button className="rounded-lg bg-[#032489] px-5 py-2.5 text-sm font-black text-white">Confirm order</button></form></div>
      </section>}

      {order.commercial_state === 'draft' && firstItem && <div className="mb-5"><DraftSalesDetails order={order} item={firstItem} users={users} /></div>}
      ...
      <div className="mb-5 grid gap-4 lg:grid-cols-3">
        ...
        <Card title="Price" help="The selling-price snapshot stays fixed on the Order even if website prices change later."><MoneyRow label="Items" value={order.subtotal} />{order.discount_amount > 0 && <MoneyRow label="Discount" value={-order.discount_amount} />}{order.cash_off_amount > 0 && <MoneyRow label="Cash-Off" value={-order.cash_off_amount} />}{order.delivery_charge > 0 && <MoneyRow label="Delivery" value={order.delivery_charge} />}<div className="mt-2 border-t pt-2"><MoneyRow label="Total" value={order.total_amount} strong /></div>{firstItem?.unit_cost_snapshot != null && <p className="mt-3 text-xs text-slate-500">Internal unit cost: <strong>{money(firstItem.unit_cost_snapshot)}</strong></p>}</Card>
        <Card title="Source & commission" help="Original Ambassador credit survives later handovers. Sales staff is separate from Ambassador attribution."><p className="mt-3 text-xs text-slate-500">Source</p><p className="font-black capitalize text-slate-900">{order.acquisition_source || order.source_type}</p><p className="mt-3 text-xs text-slate-500">Sales staff</p><p className="font-bold text-slate-800">{order.sales_staff_name || 'Not recorded'}</p><p className="mt-3 text-xs text-slate-500">Ambassador</p><p className="font-bold text-slate-800">{ambassador?.name || 'No Ambassador attribution'}</p>{order.ambassador_id && <div className="mt-3 rounded-lg bg-slate-50 p-3 text-sm"><div className="flex justify-between"><span>{order.commission_rate}%</span><strong>{money(order.commission_amount || estimatedCommission)}</strong></div><p className="mt-1 text-xs capitalize text-slate-500">{order.commission_status}</p></div>}</Card>
      </div>

      <div className="mb-5"><OrderPayments order={order} payments={payments} /></div>
      {solarItem && <div className="mb-5"><SolarInstallationCard orderId={order.id} orderItemId={solarItem.id} installation={solarInstallation} users={users} /></div>}
      ...
      <div className="grid gap-5 xl:grid-cols-[1.2fr_.8fr]">
        <div className="space-y-5">
          <section className="rounded-xl border border-slate-200 bg-white shadow-sm"><div className="border-b px-5 py-4"><h2 className="text-sm font-black">Order items & fulfilment</h2></div><div className="divide-y">{(order.items || []).map((item) => <div key={item.id} className="px-5 py-4"><div className="flex justify-between gap-4"><div><p className="text-sm font-black">{item.item_name} × {item.quantity}</p><p className="mt-1 text-xs capitalize text-slate-500">{item.item_type}{item.brand ? ` · ${item.brand}` : ''}{item.model ? ` ${item.model}` : ''}</p><p className="mt-1 text-xs text-slate-500">{item.fulfilment_source === 'internal' ? `EmmyTech stock${item.source_location_id ? ` · ${locationMap.get(item.source_location_id) || 'Location'}` : ' · source required'}` : item.fulfilment_source}</p></div><div className="text-right"><p className="font-black">{money(item.line_total || Number(item.unit_price || 0) * item.quantity)}</p><p className="mt-1 text-xs text-slate-400">Reserved {item.quantity_reserved}/{item.quantity}</p></div></div>{Object.keys(item.specs || {}).length > 0 && <div className="mt-3 flex flex-wrap gap-2">{Object.entries(item.specs).filter(([,v]) => v !== '' && v != null).map(([k,v]) => <span key={k} className="rounded-md bg-slate-50 px-2 py-1 text-[11px] text-slate-600"><strong>{k.replaceAll('_',' ')}:</strong> {typeof v === 'boolean' ? (v ? 'Yes' : 'No') : String(v)}</span>)}</div>}{order.commercial_state === 'draft' ? <DraftFulfilmentSource orderId={order.id} item={item} locations={locations} /> : null}</div>)}</div></section>
          <section className="rounded-xl border border-slate-200 bg-white shadow-sm"><div className="border-b px-5 py-4"><h2 className="text-sm font-black">Order timeline</h2></div>{events.length === 0 ? <div className="p-8 text-center text-sm text-slate-400">No activity yet.</div> : <div className="divide-y">{events.map((event) => <div key={event.id} className="flex gap-3 px-5 py-4"><div className="grid h-8 w-8 place-items-center rounded-full bg-blue-50 text-[#032489]"><Clock3 className="h-4 w-4" /></div><div><p className="text-sm font-black">{event.title}</p>{event.note && <p className="mt-1 text-xs text-slate-500">{event.note}</p>}{event.event_type === 'status_skipped' && Array.isArray(event.metadata?.skipped_statuses) && <p className="mt-1 text-xs font-semibold text-amber-700">Skipped: {(event.metadata.skipped_statuses as string[]).map((value) => value.replaceAll('_',' ')).join(', ')}</p>}<p className="mt-1 text-[11px] text-slate-400">{formatDate(event.created_at)}</p></div></div>)}</div>}</section>
        </div>
        <div className="space-y-5">
...
function getNextAction(commercial: string, status: OrderStatus, reservations: number) {
  if (commercial === 'draft') return { title: 'Review & confirm this Order', detail: 'Check Sales details, Ambassador commission, price and fulfilment source, then confirm the Order.' };
  if (status === 'new') return { title: reservations > 0 ? 'Move to Stock Check' : 'Check how the Order will be fulfilled', detail: reservations > 0 ? 'Stock is reserved. Continue the fulfilment workflow or jump ahead with a reason when the real-world situation is already further along.' : 'No stock reservation exists yet. Confirm the fulfilment source before moving forward.' };
  ...
  return { title: next[status] || 'Order complete', detail: status === 'completed' || status === 'cancelled' ? 'No further action is required.' : 'Use the fulfilment control to record the next real stage. You may skip forward when needed, but a reason is required.' };
}
...
function MoneyRow({ label, value, strong = false }: { label: string; value: number; strong?: boolean }) { return <div className={`mt-2 flex justify-between text-sm ${strong ? 'font-black' : 'font-semibold'}`}><span className="text-slate-500">{label}</span><span className={strong ? 'text-slate-950' : 'text-slate-700'}>{money(value)}</span></div>; }
