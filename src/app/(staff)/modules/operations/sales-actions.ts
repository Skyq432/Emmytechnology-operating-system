...
export async function addRepairPartAction(_prev: SalesActionState, formData: FormData): Promise<SalesActionState> {
  const repairId = String(formData.get('repair_id') || '');
  const inventoryItemId = String(formData.get('inventory_item_id') || '');
  if (!repairId) return initialFail('Repair is required.');
  if (!inventoryItemId) return initialFail('Choose an inventory item.');
...
export async function removeRepairPartAction(formData: FormData) {
  const repairId = String(formData.get('repair_id') || '');
  const partId = String(formData.get('part_id') || '');
  if (!partId) return;
  const result = await removeRepairPart(partId);
  if (!result.success) throw new Error(result.message);
  if (repairId) revalidateRepair(repairId);
}
