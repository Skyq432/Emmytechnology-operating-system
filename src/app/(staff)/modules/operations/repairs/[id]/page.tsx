...
import { RepairCashOffPanel } from '@/components/operations/repairs/repair-cash-off-panel';
import { getRepairCashOffContext } from '@/lib/operations/cash-off-server';
import { getRepairAdminDetail, getRepairPartsUsed } from '@/lib/operations/repair-server';
...
export default async function RepairDetailPage({ params }:{ params:Promise<{id:string}> }) {
  ...
  const [cashOff, inventory, locations, partsUsed] = await Promise.all([
    getRepairCashOffContext(r.identity_id),
    getOperationsInventory(),
    getOperationsLocations(),
    getRepairPartsUsed(id),
  ]);
