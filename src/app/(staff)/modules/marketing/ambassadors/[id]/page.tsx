...
interface ReferralAttempt {
  ...
  last_seen_at: string;
  owner_ambassador_id: string | null;
  lead_id: string | null;
}
...
export default function AmbassadorDetailPage() {
  ...
  const [actionLeads, setActionLeads] = useState<Lead[]>([]);
  const [activities, setActivities] = useState<PostActivity[]>([]);
  const [conversions, setConversions] = useState<Conversion[]>([]);
  const [allConversions, setAllConversions] = useState<Conversion[]>([]);
  ...
  const [activeTab, setActiveTab] = useState<
    'overview' | 'leads' | 'referrals' | 'activity' | 'conversions' | 'payouts'
  >('overview');
  ...
  }, [ambassadorId, range.startIso, range.endExclusiveIso]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const { data: actionLeadsData } = await supabase
        .from('leads')
        .select('*')
        .eq('ambassador_id', ambassadorId)
        .order('created_at', { ascending: false });

      setActionLeads(actionLeadsData || []);

      const { data: referralData } = await supabase
        .from('ambassador_referral_attempts')
        .select('id, visitor_id, referral_code, status, person_label, match_reason, attempt_count, first_seen_at, last_seen_at, owner_ambassador_id, lead_id')
        .eq('attempted_ambassador_id', ambassadorId)
        .gte('first_seen_at', range.startIso)
        .lt('first_seen_at', range.endExclusiveIso)
        .order('last_seen_at', { ascending: false });

      setReferralAttempts((referralData || []) as ReferralAttempt[]);
  ...
  const softDeleteAmbassador = async () => {
    ...
    try {
      ...
      router.push('/modules/marketing/ambassadors');
    } catch (err: any) {
  ...
  const tabs = [
    ...
    { key: 'referrals', label: `Referral trail (${referralAttempts.length})`, icon: Shield },
  ...
  const selectedLead = actionLeads.find((lead) => lead.id === selectedLeadId) || null;
  const selectedLeadConversionCount = selectedLeadId
    ? allConversions.filter((conversion) => conversion.lead_id === selectedLeadId).length
    : 0;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-blue-950 via-emmy-primary to-blue-700 p-8 text-white shadow-xl">
        ...
        <button
          onClick={() => router.push('/modules/marketing/ambassadors')}
        ...
        <div className="relative grid gap-6 xl:grid-cols-[minmax(0,1fr)_560px] xl:items-end">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            <HeroStat label={`Approved Leads · ${range.label}`} value={String(leads.filter((lead) => lead.approved_as_lead === true).length)} />
            <HeroStat label={`Conversions · ${range.label}`} value={String(conversions.length)} />
            <HeroStat label="Current Balance" value={formatCurrency(ambassador.available_balance)} />
            <HeroStat label={`Paid · ${range.label}`} value={formatCurrency(periodPaid)} />
          </div>
        </div>
      </div>
          ...
          <div className="space-y-6 lg:col-span-2">
            ...
            <Card className="rounded-3xl border-0 shadow-sm">
              ...
              <CardContent className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                <Summary icon={DollarSign} label="Current Balance" value={formatCurrency(ambassador.available_balance)} />
                <Summary icon={Send} label={`Paid · ${range.label}`} value={formatCurrency(periodPaid)} />
                <Summary icon={TrendingUp} label={`Commission · ${range.label}`} value={formatCurrency(conversions.reduce((total, conversion) => total + Number(conversion.commission_amount || 0), 0))} />
                <Summary icon={Gift} label={`Bonuses · ${range.label}`} value={formatCurrency(bonuses.reduce((total, bonus) => total + Number(bonus.amount || 0), 0))} />
              </CardContent>
            </Card>
      ...
      {activeTab === 'referrals' && (
        <List empty="No tracked referral attempts yet">
          {referralAttempts.map((attempt) => (
            <Row
              key={attempt.id}
              title={attempt.person_label || `Visitor ${attempt.visitor_id.slice(0, 12)}…`}
              subtitle={`${attempt.referral_code} · ${attempt.attempt_count} ${attempt.attempt_count === 1 ? 'attempt' : 'attempts'} · ${attempt.match_reason || 'awaiting identity'} · Last seen ${formatDate(attempt.last_seen_at)}`}
        ...
        <Modal title="Add Conversion">
          ...
          >
            ...
            {actionLeads.map((lead) => {
              const leadConversionCount = allConversions.filter(
