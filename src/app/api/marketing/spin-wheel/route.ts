...
// a from/to date range (or a status/page filter), and Netlify's CDN caches GET
// responses by URL when no Cache-Control is set, with no awareness that "this month"
// means something different as time passes. Without this, the first request for a
// given set of params gets cached and every later identical request (including ones
// made after a real bug fix here) silently keeps getting served that same stale
// snapshot forever.
...
// current period at all. Paginate with .range() until every row up to `limit` is
...
function collectWarnings(results: Array<{ warning: string | null }>) {
  return results.map((result) => result.warning).filter(Boolean) as string[];
}

function playerName(player: Row | undefined, identity: Row | undefined): string {
  const value =
    identity?.primary_name ?? identity?.full_name ?? player?.full_name ?? player?.name;
  return value ? textValue(value) : "Unnamed player";
}

function playerPhone(player: Row | undefined, identity: Row | undefined): string {
  const value =
    identity?.primary_phone ?? identity?.phone_number ?? player?.phone_number ?? player?.phone;
...
function normalizeSettings(rows: Row[]) {
  return rows.map((row) => ({
    ...row,
    setting_key: row.setting_key ?? row.key ?? row.name ?? row.id,
    setting_value:
      row.setting_value ?? row.value ?? row.config_value ?? row.settings_value ?? null,
  }));
}
...
async function sanitizeRecord(table: string, record: Row) {
  ...
  return Object.fromEntries(
    Object.entries(record).filter(
      ([key, value]) => key !== "id" && keys.has(key) && value !== undefined
    )
  );
}
...
async function saveRecord(table: string, rawRecord: Row) {
  ...
  const query = id ? db.from(table).update(record).eq("id", id) : db.from(table).insert(record);
...
// ---------------------------------------------------------------------------
// Section handlers — each backs one Spin Wheel Console tab. These were folded
// into one flat GET handler at some point and the admin client never got
// updated to match, so every tab silently rendered zeroes/empties from then
// on. Restored against the live table names/columns, which never changed.
// ---------------------------------------------------------------------------
...
async function overview(fromIso: string, toIso: string) {
  ...
  const transactions = within(transactionsResult.rows, fromIso, toIso);
  const userPrizes = userPrizesResult.rows;
  const challenges = challengesResult.rows;
  const cashouts = cashoutsResult.rows;
  const referralAwards = within(referralAwardsResult.rows, fromIso, toIso);
  const shareBonusClaims = within(shareBonusResult.rows, fromIso, toIso);

  const activePlayers = new Set(spins.map((row) => rowPlayerId(row)).filter(Boolean)).size;

  const cashOffCredited = transactions
    .filter((row) => textValue(row.direction).toLowerCase() === "credit")
    .reduce((sum, row) => sum + numberValue(row.amount), 0);

  const cashOffDebited = transactions
    .filter((row) => textValue(row.direction).toLowerCase() === "debit")
    .reduce((sum, row) => sum + numberValue(row.amount), 0);

  const currentCashOff = accounts.reduce((sum, row) => sum + numberValue(row.balance), 0);

  const pendingClaims = userPrizes.filter((row) => {
    const status = textValue(row.status).toLowerCase();
    const type = textValue(row.result_type ?? row.prize_type).toLowerCase();
  ...
  for (const spin of spins) {
    const label = textValue(spin.result_label ?? spin.prize_label) || "Unknown";
  ...
  for (const item of daily) item.count = dailyMap.get(item.day) || 0;

  const referrerIds = new Set(
    referrals
      .map((row) => row.referrer_spin_player_id ?? row.referrer_player_id ?? row.referrer_id)
      .filter(Boolean)
  );

  const referralRate = activePlayers > 0 ? (referrerIds.size / activePlayers) * 100 : 0;
  const averageReferrals = referrerIds.size > 0 ? referrals.length / referrerIds.size : 0;
  ...
  return {
    ...
    deployment: {
      dailyEntitlement: hasColumn(playersResult.rows, "daily_spin_available"),
      shareBonus: tableIsAvailable(shareBonusResult),
    },
  };
}

async function rewardOperations(requestedStatus: string, requestedPage: number) {
  const [
    challengesResult,
    creditsResult,
    cashoutsResult,
  ...
  const identityMap = new Map(identitiesResult.rows.map((row) => [String(row.id), row]));

  const withPlayer = (row: Row): Row => {
    const player = playerMap.get(String(rowPlayerId(row)));
    const identity = identityMap.get(String(row.identity_id ?? player?.identity_id));
  ...
  const matchingChallenges = allChallenges.filter(
    (row) => normalizedStatus === "all" || textValue(row.status).toLowerCase() === normalizedStatus
  );
  const pageSize = 24;
  const pageCount = Math.max(1, Math.ceil(matchingChallenges.length / pageSize));
  const page = Math.min(Math.max(1, Math.trunc(requestedPage || 1)), pageCount);
  const challengeRows = matchingChallenges.slice((page - 1) * pageSize, page * pageSize);

  const cashouts = newestFirst(cashoutsResult.rows).slice(0, 100).map(withPlayer);
  ...
  const convertedChallenges = allChallenges.filter(
    (row) => textValue(row.status).toLowerCase() === "converted_to_cash_off"
  );
  ...
  const referrerFor = (playerId: string) => {
    if (!referrerMap.has(playerId)) {
      ...
      const identity = identityMap.get(String(player?.identity_id));
  ...
  for (const referral of referralsResult.rows) {
    const playerId = textValue(
      referral.referrer_spin_player_id ?? referral.referrer_player_id ?? referral.referrer_id
    );
    if (playerId) referrerFor(playerId).registrations += 1;
  }

  for (const award of referralAwardsResult.rows) {
    const playerId = textValue(award.referrer_spin_player_id ?? award.referrer_player_id);
  ...
  const playerIdByCode = new Map(
    players
      ...
      .map((row) => [textValue(row.referral_code).toLowerCase(), String(row.id)])
  );
  ...
  for (const open of referralLinkOpens) {
    const playerId = playerIdByCode.get(textValue(open.code).toLowerCase());
  ...
  const topReferrers = Array.from(referrerMap.values())
    .sort(
      (a, b) =>
        b.registrations - a.registrations || b.link_opens - a.link_opens || b.bonus_spins - a.bonus_spins
    )
    .slice(0, 20);
  ...
  const referralAwards = newestFirst(referralAwardsResult.rows).map((row) =>
    withPlayer({
      ...row,
      spin_player_id: row.referrer_spin_player_id ?? row.referrer_player_id,
      identity_id: row.referrer_identity_id ?? row.identity_id,
    })
  );
  const recentBonuses = [
    ...shareClaims.slice(0, 60).map((row) => ({ ...row, bonus_type: "share", spins_awarded: 1 })),
  ...
  const guideStarted = guideEvents.filter((row) => row.event_type === "spin_reward_guide_started").length;
  const guideCompleted = guideEvents.filter(
    (row) => row.event_type === "spin_reward_guide_completed"
  ).length;
  const guideSkipped = guideEvents.filter((row) => row.event_type === "spin_reward_guide_skipped").length;

  const firstTrackedOpenAt = referralLinkOpens.reduce((earliest, row) => {
    const time = timestampOf(row);
    return time && (!earliest || time < earliest) ? time : earliest;
  }, 0);
  const referralRegistrations = firstTrackedOpenAt
    ? referralsResult.rows.filter((row) => timestampOf(row) >= firstTrackedOpenAt).length
    : 0;
  ...
  return {
    metrics: {
      ...
      activeChallengeCash: activeChallenges.reduce((sum, row) => sum + numberValue(row.cash_balance), 0),
      cashEligible: eligibleChallenges.length,
      cashPayoutDue: eligibleChallenges.reduce((sum, row) => sum + numberValue(row.cash_balance), 0),
      ...
      pendingCashoutAmount: pendingCashouts.reduce((sum, row) => sum + numberValue(row.amount), 0),
      ...
      guideCompleted,
      guideSkipped,
      guideCompletionRate: guideStarted ? (guideCompleted / guideStarted) * 100 : 0,
    },
...
async function users(search: string) {
  const [playersResult, accountsResult, identitiesResult] = await Promise.all([
    safeRows("spin_players"),
    safeRows("cash_off_accounts"),
    safeRows("identities"),
  ]);

  const identityMap = new Map(identitiesResult.rows.map((row) => [row.id, row]));
  const accountMap = new Map(accountsResult.rows.map((row) => [row.identity_id, row]));
  ...
  const rows = newestFirst(playersResult.rows)
    ...
    .map((player) => ({
      ...
      cashOffAccount: accountMap.get(player.identity_id) || null,
    }));

  return {
    rows,
    _warnings: collectWarnings([playersResult, accountsResult, identitiesResult]),
  };
}
...
async function userDetail(playerId: string) {
  ...
  const player = playersResult.rows.find((row) => row.id === playerId);
  if (!player) throw new Error("Spin player not found.");

  const identity = identitiesResult.rows.find((row) => row.id === player.identity_id);
  const account = accountsResult.rows.find((row) => row.identity_id === player.identity_id);
  ...
  return {
    ...
    transactions: newestFirst(
      transactionsResult.rows.filter((row) => row.identity_id === player.identity_id)
    ).slice(0, 50),
    spins: newestFirst(
      spinsResult.rows.filter((row) => (row.spin_player_id ?? row.player_id) === player.id)
    ).slice(0, 50),
    prizes: newestFirst(
      prizesResult.rows.filter((row) => (row.spin_player_id ?? row.player_id) === player.id)
    ).slice(0, 50),
    ...
    challenges: newestFirst(
      challengesResult.rows.filter(
        (row) => rowPlayerId(row) === player.id || row.identity_id === player.identity_id
      )
    ).slice(0, 20),
    ...
    shareBonusClaims: newestFirst(
      shareBonusResult.rows.filter((row) => rowPlayerId(row) === player.id)
    ).slice(0, 50),
    ...
    deployment: {
      dailyEntitlement: Object.prototype.hasOwnProperty.call(player, "daily_spin_available"),
      shareBonus: tableIsAvailable(shareBonusResult),
    },
    _warnings: collectWarnings([
      playersResult,
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (section === "overview") {
      ...
      return json({ ok: true, data: await overview(fromIso, toIso) });
    }
    ...
    if (section === "operations") {
      ...
      return json({ ok: true, data: await rewardOperations(status, page) });
    }
    ...
    if (section === "rules") {
      const [groupsResult, itemsResult, settingsResult, lettersResult] = await Promise.all([
      ...
      const groups = [...groupsResult.rows].sort(
        (a, b) => numberValue(a.priority) - numberValue(b.priority) || numberValue(a.start_spin) - numberValue(b.start_spin)
      );

      const items = [...itemsResult.rows].sort(
        (a, b) =>
          textValue(a.group_id).localeCompare(textValue(b.group_id)) ||
      ...
      return json({
        ...
        data: {
          ...
          letters: [...lettersResult.rows].sort(
            (a, b) => numberValue(a.segment_order) - numberValue(b.segment_order)
          ),
          _warnings: collectWarnings([groupsResult, itemsResult, settingsResult, lettersResult]),
        },
      });
    }
    ...
    if (section === "claims") {
      ...
      const rows = newestFirst(claimsResult.rows)
        .filter((row) => status === "all" || textValue(row.status).toLowerCase() === status.toLowerCase())
        .slice(0, 400)
        .map((row) => ({
          ...row,
          player: playerMap.get(row.spin_player_id ?? row.player_id) || null,
        }));

      return json({ ok: true, data: rows, warnings: collectWarnings([claimsResult, playersResult]) });
    }
    ...
    if (section === "cashoff") {
      ...
      const { map: identityMap, warning } = await identityMapFor(
        accountsResult.rows.map((row) => String(row.identity_id ?? "")).filter(Boolean)
      );
      ...
      return json({
        ...
        data: {
          accounts: [...accountsResult.rows]
            .sort((a, b) => numberValue(b.balance) - numberValue(a.balance))
            .slice(0, 400)
            .map((row) => ({ ...row, identity: identityMap.get(String(row.identity_id)) || null })),
          transactions: newestFirst(transactionsResult.rows).slice(0, 400),
          _warnings: [...collectWarnings([accountsResult, transactionsResult]), ...(warning ? [warning] : [])],
        },
      });
    }
    ...
    if (section === "users") {
      ...
      return json({ ok: true, data: result.rows, warnings: result._warnings });
    }

    if (section === "user") {
      const playerId = searchParams.get("playerId") || "";
      return json({ ok: true, data: await userDetail(playerId) });
    }
    ...
    if (section === "logs") {
      ...
      const rows = newestFirst(result.rows)
        .filter(
          (row) =>
            resultType === "all" || textValue(row.result_type).toLowerCase() === resultType.toLowerCase()
        )
        .slice(0, 500);

      return json({ ok: true, data: rows, warnings: result.warning ? [result.warning] : [] });
    }

    return json({ ok: false, error: "Unknown admin section." }, { status: 400 });
  } catch (error) {
    return json(
      { ok: false, error: error instanceof Error ? error.message : "The admin request failed." },
      { status: 500 }
    );
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (action === "save_prize") {
      const input = (body.record ?? {}) as Row;
      const record = {
        ...input,
        label: textValue(input.label),
        prize_type: textValue(input.prize_type) || "cash",
        gravity: numberValue(input.gravity || 1),
        stock: numberValue(input.stock),
        monetary_value: numberValue(input.monetary_value),
        is_active: Boolean(input.is_active),
        on_wheel: Boolean(input.on_wheel),
        near_miss: Boolean(input.near_miss),
      };

      if (!record.label) throw new Error("Prize label is required.");

      return json({ ok: true, data: await saveRecord("spin_prizes", record) });
    }

    if (action === "delete_prize") {
      await deleteRecord("spin_prizes", textValue(body.id));
      return json({ ok: true });
    }
    ...
    if (action === "save_group") {
      ...
      const record = {
        ...input,
        group_key: textValue(input.group_key),
        group_name: textValue(input.group_name),
        group_type: textValue(input.group_type) || "weighted",
        start_spin: Math.max(1, numberValue(input.start_spin)),
        end_spin:
          input.end_spin === "" || input.end_spin === null || input.end_spin === undefined
            ? null
            : numberValue(input.end_spin),
      ...
      if (!record.group_key || !record.group_name) {
        throw new Error("Group key and group name are required.");
      }

      return json({ ok: true, data: await saveRecord("spin_rule_groups", record) });
    }

    if (action === "delete_group") {
      const groupId = textValue(body.id);
      const { error: itemError } = await db.from("spin_rule_items").delete().eq("group_id", groupId);
      if (itemError) throw new Error(itemError.message);

      await deleteRecord("spin_rule_groups", groupId);
      return json({ ok: true });
    }
    ...
    if (action === "save_item") {
      ...
      const record = {
        ...input,
        group_id: textValue(input.group_id),
        item_key: textValue(input.item_key),
        result_label: textValue(input.result_label),
        result_type: textValue(input.result_type) || "cash",
        cash_amount: numberValue(input.cash_amount),
        letter_code: textValue(input.letter_code) || null,
        bonus_spins: numberValue(input.bonus_spins),
        gravity: numberValue(input.gravity || 1),
        item_order: numberValue(input.item_order || 1),
        max_uses_per_user: numberValue(input.max_uses_per_user || 999),
        is_active: Boolean(input.is_active),
        updated_at: new Date().toISOString(),
      };

      if (!record.group_id || !record.item_key || !record.result_label) {
        throw new Error("Group, item key and result label are required.");
      }

      return json({ ok: true, data: await saveRecord("spin_rule_items", record) });
    }

    if (action === "delete_item") {
      await deleteRecord("spin_rule_items", textValue(body.id));
      return json({ ok: true });
    }

    if (action === "save_setting") {
      const sample = await tableSample("spin_game_settings");

      const keyColumn = sample && "setting_key" in sample ? "setting_key" : sample && "key" in sample ? "key" : "setting_key";
      const valueColumn =
        sample && "setting_value" in sample ? "setting_value" : sample && "value" in sample ? "value" : "setting_value";
      ...
      const record: Row = { [keyColumn]: body.key, [valueColumn]: body.value };
      if (!sample || "updated_at" in sample) record.updated_at = new Date().toISOString();

      const query =
        existing && existing.length
          ? db.from("spin_game_settings").update(record).eq(keyColumn, body.key)
    ...
    if (action === "claim_status") {
      ...
      const rawRecord: Row = {
        id: body.id,
        status: nextStatus,
        claimed_at: nextStatus === "claimed" ? new Date().toISOString() : null,
      };

      return json({ ok: true, data: await saveRecord("spin_user_prizes", rawRecord) });
    }
    ...
    if (action === "cashout_status") {
      ...
      const adminNote = [auditLine, textValue(current.admin_note)].filter(Boolean).join("\n").slice(0, 4000);
      ...
      return json({ ok: true, data, warnings: auditError ? [`Audit timeline: ${auditError.message}`] : [] });
    }
    ...
    if (action === "add_spins") {
      ...
      if (!Number.isInteger(amount) || amount === 0) {
        throw new Error("Spin adjustment must be a non-zero whole number.");
      }
      ...
      const next = Math.max(0, numberValue(player.spins_remaining) + amount);
      const update: Row = { spins_remaining: next };
      if ("updated_at" in player) update.updated_at = new Date().toISOString();
    ...
    if (action === "cashoff_adjust") {
      ...
      if (amount === 0) throw new Error("Cash Off adjustment cannot be zero.");
      ...
      const args = {
        ...
        p_spin_log_id: null,
        p_created_by: null,
        p_reason: textValue(body.reason) || "Admin adjustment",
        p_metadata: { panel: "emmy40" },
        p_idempotency_key: textValue(body.idempotencyKey) || `emmy40-${body.identityId}-${Date.now()}`,
      };
      ...
      for (const functionName of functionNames) {
        ...
        if (!error) return json({ ok: true, data, functionName });
        errors.push(`${functionName}: ${error.message}`);
      }

      throw new Error(`Cash Off adjustment function was not found. ${errors.join(" | ")}`);
    }

    return json({ ok: false, error: "Unknown admin action." }, { status: 400 });
  } catch (error) {
    return json(
      { ok: false, error: error instanceof Error ? error.message : "The admin action failed." },
      { status: 500 }
    );
  }
}
