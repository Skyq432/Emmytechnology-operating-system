...
export function OrdersClient({
  ...
  const totals = useMemo(
    () => calculateOrderTotals({ subtotal: unitPrice * quantity, discountAmount, cashOffAmount, deliveryCharge }),
    ...
    <div className="mx-auto max-w-[1500px]">
      <div className="mb-5 flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-500">Create a draft first. Stock and commission only change when the order is confirmed.</p>
        </div>
      ...
      {showCreate && (
        <form action={formAction} className="mb-5 space-y-5 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <section className="rounded-xl bg-slate-50 p-4">
            <div className="mb-3 flex items-center gap-2"><p className="text-xs font-black uppercase tracking-wide text-slate-500">1. Customer</p><HelpTip text="Type a phone number or name. If EmmyTech already knows the person, choose the match so this order connects to the same CRM Identity." label="About customer matching" /></div>
              ...
              {identityResults.length > 0 && (
                <div className="absolute z-20 mt-1 w-full overflow-hidden rounded-lg border border-slate-200 bg-white shadow-lg">
                  {identityResults.map((identity) => (
                    <button type="button" key={identity.id} onClick={() => chooseIdentity(identity)} className="flex w-full items-start justify-between gap-3 border-b border-slate-100 px-4 py-3 text-left last:border-0 hover:bg-blue-50">
                      <div><p className="text-sm font-bold text-slate-800">{identity.primary_name || identity.primary_phone || identity.identity_code}</p><p className="mt-1 text-xs text-slate-500">{identity.primary_phone || 'No phone'} · {identity.primary_email || 'No email'}</p></div>
                      <span className="rounded-full bg-blue-50 px-2 py-1 text-[10px] font-black text-[#032489]">Stage {identity.crm_stage || '—'} {identity.crm_stage_name}</span>
                    </button>
                  ))}
                </div>
              )}
            ...
            {selectedIdentity && (
              <div className="mt-3 flex flex-col justify-between gap-3 rounded-lg border border-blue-100 bg-white p-3 sm:flex-row sm:items-center">
                <div className="flex items-start gap-3"><UserCheck className="mt-0.5 h-4 w-4 text-[#032489]" /><div><p className="text-sm font-black text-slate-800">Using existing Identity: {selectedIdentity.identity_code}</p><p className="mt-1 text-xs text-slate-500">CRM: Stage {selectedIdentity.crm_stage} {selectedIdentity.crm_stage_name}{selectedIdentity.ambassador_name ? ` · Ambassador: ${selectedIdentity.ambassador_name}` : ''} · Cash-Off: {money(selectedIdentity.cash_off_balance)}</p></div></div>
                <button type="button" onClick={clearIdentity} className="text-xs font-bold text-slate-500 hover:text-[#032489]">Use someone else</button>
              </div>
            )}
            ...
            <input type="hidden" name="ambassador_id" value={selectedIdentity?.ambassador_id || ''} />
            <input type="hidden" name="acquisition_source" value={selectedIdentity?.acquisition_source || ''} />
            <div className="mt-4 grid gap-4 md:grid-cols-3">
              <Field label="Customer name"><input name="customer_name" value={customerName} onChange={(e) => setCustomerName(e.target.value)} className="input" /></Field>
              <Field label="Phone"><input name="customer_phone" value={customerPhone} onChange={(e) => setCustomerPhone(e.target.value)} className="input" /></Field>
              <Field label="Email"><input name="customer_email" value={customerEmail} onChange={(e) => setCustomerEmail(e.target.value)} className="input" /></Field>
            </div>
          </section>
          ...
          <section className="rounded-xl border border-slate-200 p-4">
            <div className="mb-3 flex items-center gap-2"><p className="text-xs font-black uppercase tracking-wide text-slate-500">3. Money & attribution</p><HelpTip text="These values are frozen on this order. Later price or commission changes will not rewrite an old order." label="About order money" /></div>
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              ...
              <Field label="Cash-Off to apply"><input name="cash_off_amount" type="number" min="0" max={selectedIdentity?.cash_off_balance || undefined} value={cashOffAmount || ''} onChange={(e) => setCashOffAmount(Number(e.target.value || 0))} className="input" /></Field>
              ...
              <Field label="Ambassador commission %"><input name="commission_rate" type="number" min="0" step="0.01" value={commissionRate || ''} onChange={(e) => setCommissionRate(Number(e.target.value || 0))} className="input" disabled={!selectedIdentity?.ambassador_id} /></Field>
            ...
            <div className="mt-4 grid gap-3 rounded-lg bg-slate-50 p-4 sm:grid-cols-4">
              <Money label="Items" value={unitPrice * quantity} />
              <Money label="Discount + Cash-Off" value={discountAmount + cashOffAmount} />
              <Money label="Delivery" value={deliveryCharge} />
              <Money label="Draft total" value={totals.totalAmount} strong />
            </div>
            {selectedIdentity?.ambassador_id && <p className="mt-3 text-xs text-slate-500">Pending commission after confirmation: <strong>{money(totals.totalAmount * commissionRate / 100)}</strong> at {commissionRate}%.</p>}
          </section>

          <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <Field label="Reference"><input name="reference_label" className="input" placeholder="e.g. WhatsApp order" /></Field>
            <Field label="Source"><select name="source_type" className="input" defaultValue={selectedIdentity ? 'crm' : 'manual'}><option value="manual">Manual</option><option value="crm">CRM</option><option value="website">Website</option><option value="whatsapp">WhatsApp</option><option value="internal">Internal</option><option value="other">Other</option></select></Field>
            <Field label="Source reference"><input name="source_reference" className="input" placeholder="Lead / chat / campaign ref" /></Field>
            <Field label="Due at"><input name="due_at" type="datetime-local" className="input" /></Field>
            <input type="hidden" name="current_team" value="Operations" />
          </section>
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        {filtered.length === 0 ? <div className="py-14 text-center"><ClipboardList className="mx-auto h-8 w-8 text-slate-300" /><p className="mt-3 text-sm font-bold text-slate-700">No matching orders</p></div> : (
          <div className="overflow-x-auto"><table className="w-full min-w-[980px] text-left text-sm"><thead className="bg-slate-50 text-[11px] uppercase tracking-wide text-slate-500"><tr><th className="px-5 py-3">Order</th><th className="px-5 py-3">Customer</th><th className="px-5 py-3">Commercial</th><th className="px-5 py-3">Total</th><th className="px-5 py-3">Fulfilment</th><th className="px-5 py-3">Commission</th><th className="px-5 py-3">Updated</th></tr></thead><tbody className="divide-y divide-slate-100">{filtered.map((order) => <tr key={order.id} className="hover:bg-slate-50/70"><td className="px-5 py-4 font-black"><Link href={`/modules/operations/orders/${order.id}`} className="text-[#032489] hover:underline">{order.order_code}</Link></td><td className="px-5 py-4"><div className="font-bold text-slate-800">{order.customer_name || order.reference_label || 'Internal order'}</div><div className="mt-1 text-xs text-slate-400">{order.customer_phone || order.source_type}</div></td><td className="px-5 py-4"><span className={`rounded-full px-2.5 py-1 text-xs font-black ${order.commercial_state === 'confirmed' ? 'bg-blue-50 text-[#032489]' : 'bg-slate-100 text-slate-600'}`}>{order.commercial_state}</span></td><td className="px-5 py-4 font-black text-slate-800">{money(order.total_amount)}</td><td className="px-5 py-4 text-slate-600">{getOrderStatusLabel(order.status)}</td><td className="px-5 py-4 text-slate-600">{order.commission_status === 'none' ? '—' : `${money(order.commission_amount)} · ${order.commission_status}`}</td><td className="px-5 py-4 text-xs font-semibold text-slate-400">{new Date(order.updated_at).toLocaleString('en-NG', { dateStyle: 'medium', timeStyle: 'short' })}</td></tr>)}</tbody></table></div>
        )}
      </div>
      <style jsx>{`.input{width:100%;border:1px solid #e2e8f0;border-radius:8px;padding:10px 12px;font-size:14px;outline:none;background:#fff}.input:focus{border-color:#032489;box-shadow:0 0 0 3px rgba(3,36,137,.08)}.input:disabled{background:#f8fafc;color:#94a3b8}`}</style>
    </div>
  );
}
...
function Money({ label, value, strong = false }: { label: string; value: number; strong?: boolean }) { return <div><p className="text-[10px] font-black uppercase tracking-wide text-slate-400">{label}</p><p className={`mt-1 ${strong ? 'text-lg text-[#032489]' : 'text-sm text-slate-800'} font-black`}>{money(value)}</p></div>; }
