  ...
  return (
    <div className="mx-auto max-w-[1500px]">
      <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.18em] text-[#0d7a4b]">Internal execution</p>
          <h1 className="mt-2 text-3xl font-black tracking-[-0.04em]">Orders</h1>
          <p className="mt-2 text-sm text-slate-500">Track who owns each order and where it is inside the in-house workflow.</p>
        </div>
        <button
      ...
      {showCreate && (
        <form action={formAction} className="mb-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="mb-4">
            <h2 className="font-black text-slate-900">Create internal order</h2>
            <p className="mt-1 text-xs text-slate-500">The item can be internal inventory, a website-only product, or a completely manual item.</p>
          </div>
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            ...
            <Field label="Source">
              <select name="source_type" className="input" defaultValue="manual">
                <option value="manual">Manual</option><option value="crm">CRM</option><option value="website">Website</option><option value="whatsapp">WhatsApp</option><option value="internal">Internal</option><option value="other">Other</option>
              </select>
            </Field>
          ...
          <div className="mt-5 border-t border-slate-100 pt-5">
            <p className="mb-3 text-xs font-black uppercase tracking-[0.14em] text-slate-400">First order item</p>
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              ...
              <Field label="Internal inventory (optional)">
                <select name="inventory_item_id" className="input" defaultValue="">
                  <option value="">No inventory link</option>
                  {inventory.map((item) => <option key={item.id} value={item.id}>{item.sku} · {item.name}</option>)}
                </select>
              </Field>
              <Field label="Website product (optional)">
                <select name="website_product_id" className="input" defaultValue="">
                  <option value="">No website link</option>
                  {websiteProducts.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}
                </select>
              </Field>
            </div>
          </div>
      ...
      <div className="mb-4 flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:flex-row">
        <div className="flex flex-1 items-center gap-2 rounded-xl border border-slate-200 px-3">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} className="w-full bg-transparent py-2.5 text-sm outline-none" placeholder="Search order, customer, team, reference..." />
        </div>
        <select value={status} onChange={(e) => setStatus(e.target.value)} className="rounded-xl border border-slate-200 px-3 py-2.5 text-sm font-semibold outline-none">
          <option value="all">All statuses</option>
          {['new','confirmed','stock_check','assigned','picking','packing','ready_dispatch','dispatched','delivered','completed','on_hold','cancelled'].map((value) => <option key={value} value={value}>{value.replaceAll('_', ' ')}</option>)}
        </select>
      </div>

      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        {filtered.length === 0 ? (
          <div className="py-16 text-center"><ClipboardList className="mx-auto h-9 w-9 text-slate-300" /><p className="mt-3 text-sm font-bold text-slate-700">No matching orders</p></div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full min-w-[900px] text-left text-sm">
              <thead className="bg-slate-50 text-[11px] uppercase tracking-wide text-slate-500">
                <tr><th className="px-5 py-3">Order</th><th className="px-5 py-3">Customer / reference</th><th className="px-5 py-3">Status</th><th className="px-5 py-3">Team</th><th className="px-5 py-3">Priority</th><th className="px-5 py-3">Items</th><th className="px-5 py-3">Updated</th></tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {filtered.map((order) => (
                  <tr key={order.id} className="hover:bg-slate-50/70">
                    ...
                    <td className="px-5 py-4"><span className={`font-black capitalize ${order.priority === 'urgent' ? 'text-rose-600' : 'text-slate-600'}`}>{order.priority}</span></td>
                    <td className="px-5 py-4 text-slate-600">{order.items?.reduce((sum, item) => sum + item.quantity, 0) || 0}</td>
                    <td className="px-5 py-4 text-xs font-semibold text-slate-400">{new Date(order.updated_at).toLocaleString('en-NG', { dateStyle: 'medium', timeStyle: 'short' })}</td>
                  </tr>
                ))}
              </tbody>
            </table>
