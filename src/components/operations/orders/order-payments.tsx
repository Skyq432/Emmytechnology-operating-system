...
type OrderWithReceipt = OperationsOrder & {
...
export function OrderPayments({order,payments}:{order:OperationsOrder;payments:OperationsOrderPayment[]}){
 ...
 const receiptOrder = order as OrderWithReceipt;
 const finalReceiptId = receiptOrder.final_receipt_id;
 return <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
   <div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-2"><h2 className="text-sm font-black text-slate-900">Payments</h2><HelpTip text="Record each payment separately. The Order automatically updates Paid, Balance and payment status. Pending Ambassador commission becomes earned only when the Order is fully paid." label="About payments" /></div>{finalReceiptId?<a href={`/api/sales/documents/${finalReceiptId}`} target="_blank" rel="noreferrer" className="rounded-lg bg-emerald-700 px-3 py-2 text-xs font-black text-white">View Final Receipt</a>:null}</div>
   ...
   {finalReceiptId&&receiptOrder.final_receipt_number?<p className="mt-3 text-xs font-semibold text-emerald-700">Final receipt {receiptOrder.final_receipt_number} is available.</p>:null}
