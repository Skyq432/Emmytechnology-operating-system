...
export function OrderPayments({order,payments}:{order:OperationsOrder;payments:OperationsOrderPayment[]}){
 ...
 return <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
   <div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-2"><h2 className="text-sm font-black text-slate-900">Payments</h2><HelpTip text="Record each payment separately. The Order automatically updates Paid, Balance and payment status. Pending Ambassador commission becomes earned only when the Order is fully paid." label="About payments" /></div><div className="flex items-center gap-3">{order.final_receipt_id?<SendReceiptButton documentId={order.final_receipt_id} defaultEmail={order.customer_email} label="Send Final Receipt" />:null}{order.final_receipt_id?<a href={`/api/sales/documents/${order.final_receipt_id}`} target="_blank" rel="noreferrer" className={buttonVariants({ variant: 'success', size: 'sm' })}>View Final Receipt</a>:null}</div></div>
   ...
   {payments.length>0&&<div className="mt-4 divide-y divide-slate-100 rounded-lg border border-slate-100">{payments.map(p=><div key={p.id} className="flex flex-col justify-between gap-2 px-3 py-3 sm:flex-row sm:items-center"><div><p className="text-sm font-black text-slate-800">{money(p.amount)}</p><p className="mt-1 text-xs text-slate-500 capitalize">{p.payment_method.replaceAll('_',' ')}{p.reference?` · ${p.reference}`:''}</p></div><div className="flex items-center gap-3"><p className="text-xs font-semibold text-slate-400">{new Date(p.paid_at).toLocaleString('en-NG',{dateStyle:'medium',timeStyle:'short'})}</p>{!p.is_void&&<SendReceiptButton sourceType="order" sourcePaymentId={p.id} defaultEmail={order.customer_email} />}</div></div>)}</div>}
