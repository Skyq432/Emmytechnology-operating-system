...
const ambassadorGroups: NavGroup[] = [
  { label: 'Ambassador programme', items: [
  ...
  { label: 'Performance & pipeline', items: [
...
export function DashboardSidebar({ role = 'ambassador', user }: { role?: string; user?: any }) {
  ...
  const contextualGroups: NavGroup[] = isSpinWheel
    ? [{ label: 'Spin Wheel', items: [{ label: 'Spin Wheel Console', href: '/modules/marketing/spin-wheel', icon: CircleGauge }] }]
    : isAmbassadorSolution
      ? ambassadorGroups
      : [{ label: 'Active solutions', items: [
  ...
  useEffect(() => {
    document.documentElement.dataset.ambassadorSidebar = collapsed ? 'collapsed' : 'expanded';
    window.localStorage.setItem('emmytech-marketing-sidebar', collapsed ? 'collapsed' : 'expanded');
  }, [collapsed]);
  ...
  const initials = useMemo(() => {
    ...
    return parts.length > 1 ? `${parts[0][0]}${parts[parts.length - 1][0]}`.toUpperCase() : displayName.slice(0, 2).toUpperCase();
  }, [displayName]);
    ...
    <div className={`flex h-full flex-col overflow-hidden bg-gradient-to-b from-[#073b9f] to-[#073287] text-white ${isMobile ? 'rounded-r-[28px] shadow-2xl' : 'rounded-r-[28px] shadow-[8px_0_24px_rgba(15,23,42,0.08)]'}`}>
      <div className="flex items-center gap-3 px-4 pb-3 pt-4">
        <div className={`flex h-[66px] min-w-0 items-center overflow-hidden rounded-[18px] bg-white px-3 shadow-sm ${collapsed && !isMobile ? 'w-12 justify-center' : 'flex-1'}`}>
          <img src="/emmytech-logo.png" alt="EmmyTech" className={`object-contain ${collapsed && !isMobile ? 'h-9 w-10' : 'h-12 w-full'}`} />
        </div>
        <button
        ...
        >
          {isMobile ? <X className="h-5 w-5" /> : collapsed ? <ChevronRight className="h-4 w-4" /> : <ChevronLeft className="h-4 w-4" />}
        </button>
      </div>
      ...
      {(!collapsed || isMobile) && (
        <div className="px-4 pb-2">
          <div className="flex items-center gap-2 rounded-xl bg-white/[0.08] px-3 py-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-blue-100/[0.85]">
            ...
            {isStaff ? workspaceLabel : 'Ambassador workspace'}
          </div>
        </div>
      )}
      ...
      <nav className="ambassador-nav-scroll flex-1 overflow-y-auto px-3 py-2">
        <div className="pb-2">
          {marketingNavigation.map((item) => <SidebarLink key={item.href} item={item} active={isActive(item.href)} collapsed={collapsed && !isMobile} onNavigate={() => setMobileOpen(false)} />)}
        </div>
        {contextualGroups.map((group) => (
          <section key={group.label} className="border-t border-white/10 px-1 pb-2 pt-4">
            {(!collapsed || isMobile) && <p className="mb-2 px-3 text-[10px] font-bold uppercase tracking-[0.16em] text-blue-100/45">{group.label}</p>}
            {group.items.map((item) => <SidebarLink key={item.href} item={item} active={isActive(item.href)} collapsed={collapsed && !isMobile} onNavigate={() => setMobileOpen(false)} />)}
          </section>
        ))}
      </nav>

      <div className="m-3 mt-2 flex items-center gap-2 rounded-2xl border border-white/10 bg-white/[0.07] p-2">
        <div className="grid h-10 w-10 shrink-0 place-items-center overflow-hidden rounded-xl bg-white text-xs font-bold text-emmy-primary">{initials}</div>
        {(!collapsed || isMobile) && (
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-semibold text-white">{displayName}</p>
            <p className="truncate text-[11px] text-blue-100/[0.65]">{roleLabel(role)}</p>
          </div>
        )}
        {(!collapsed || isMobile) && <button onClick={handleSignOut} className="grid h-9 w-9 shrink-0 place-items-center rounded-xl text-blue-100/[0.75] transition hover:bg-white/10 hover:text-white" aria-label="Sign out"><LogOut className="h-4 w-4" /></button>}
      </div>
    </div>
  );
  ...
  return (
    <>
      <button onClick={() => setMobileOpen(true)} className="fixed left-4 top-4 z-[60] grid h-10 w-10 place-items-center rounded-xl border border-slate-200 bg-white text-emmy-primary shadow-sm lg:hidden" aria-label="Open menu"><Menu className="h-5 w-5" /></button>
      {mobileOpen && (
...
function SidebarLink({ item, active, collapsed, onNavigate }: { item: NavItem; active: boolean; collapsed: boolean; onNavigate: () => void }) {
  const Icon = item.icon;
  return (
    <Link href={item.href} onClick={onNavigate} title={collapsed ? item.label : undefined} className={`mb-1 flex min-h-11 items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all duration-150 ${active ? 'bg-white text-emmy-primary shadow-[0_10px_26px_rgba(0,20,70,0.22)]' : 'text-blue-100/[0.80] hover:bg-white/[0.08] hover:text-white'} ${collapsed ? 'justify-center px-0' : ''}`}>
