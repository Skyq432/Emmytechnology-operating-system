...
function contact(snapshot: JsonRecord): string {
  return [snapshot.customer_phone, snapshot.customer_email]
    .filter(Boolean)
    .map(escapeLatex)
    .join(' \\textbar{} ') || 'Nigeria';
}
...
function receiptItems(snapshot: JsonRecord): Array<JsonRecord> {
  ...
  if (snapshot.source_type === 'repair') {
    const description = [snapshot.device_type, snapshot.brand, snapshot.model, snapshot.repair_type]
      .filter(Boolean)
      .join(' · ') || 'Repair service';
...
export function buildReceiptTemplateData(input: {
  ...
  const totalPaid = number(s.cumulative_paid ?? s.total_paid ?? (
    Array.isArray(s.payments)
      ? (s.payments as JsonRecord[]).reduce((sum, payment) => sum + number(payment.amount), 0)
      : 0
  ));
  const balance = Math.max(0, number(s.balance_due ?? (total - totalPaid)));
  const amountReceived = number(s.payment_amount ?? totalPaid);
  const paymentStatus = balance <= 0 && totalPaid >= total && total > 0
    ? 'Paid in Full'
    : totalPaid > 0 ? 'Part Payment' : 'Payment Recorded';
  ...
  return {
    ...
    SOURCE_REFERENCE: escapeLatex(s.source_code || input.documentNumber),
    CUSTOMER_NAME: escapeLatex(s.customer_name || 'Customer'),
    CUSTOMER_CONTACT: contact(s),
    PAYMENT_STATUS: escapeLatex(paymentStatus),
    ITEM_ROWS: itemRows(receiptItems(s), 'receipt'),
    TRANSACTION_TOTAL: money(total),
    AMOUNT_RECEIVED: money(amountReceived),
    TOTAL_PAID: money(totalPaid),
    BALANCE_DUE: money(balance),
    PAYMENT_METHOD: humanPaymentMethod(s.payment_method ?? lastPayment?.payment_method),
    PAYMENT_REFERENCE: escapeLatex(s.payment_reference ?? lastPayment?.reference ?? '—'),
    PAYMENT_DATE: date(paymentDate),
  };
}

export function buildQuotationTemplateData(input: {
  documentNumber: string;
  issuedAt: string;
  snapshot: JsonRecord;
}): Record<string, string> {
  ...
  const total = number(s.total_amount ?? s.transaction_total ?? items.reduce(
    (sum, item) => sum + number(item.line_total ?? (
      number(item.final_unit_price) * Math.max(1, number(item.quantity))
    )), 0
  ));
  const documentNumber = String(s.quotation_code || input.documentNumber);

  return {
    DOCUMENT_NUMBER: escapeLatex(documentNumber),
    QUOTATION_NUMBER: escapeLatex(documentNumber),
    VERSION: escapeLatex(s.version ?? '1'),
    DATE_ISSUED: date(input.issuedAt),
    VALID_UNTIL: calendarDate(s.validity_expires_at),
    CUSTOMER_NAME: escapeLatex(s.customer_name || 'Customer'),
    CUSTOMER_CONTACT: contact(s),
    QUOTATION_STATUS: escapeLatex(s.status || 'Published'),
    ITEM_ROWS: itemRows(items, 'quotation'),
    TRANSACTION_TOTAL: money(total),
    TOTAL: money(total),
    CUSTOMER_NOTE: escapeLatex(s.customer_note || 'Please contact Emmy Technology if you need any clarification.'),
